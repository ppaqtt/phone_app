...
### 新增
- **F17 OCR 文字识别**: 集成 Google ML Kit Text Recognition v2 中文模型
  (on-device, 无需网络 / API Key)。笔记编辑页 IMAGE 工具面板加「识别文字」
  按钮, 选图后自动识别并 append 到笔记正文; 大图自动等比缩放到 max 1920px;
  识别中 / 成功 / 失败均有 Toast 反馈。
...
### 新增
- **F16 语音转文字**: 封装 Android SpeechRecognizer (系统内置, 无需 API Key)。
  笔记编辑页底部工具栏加「语音」按钮, 点击请求 RECORD_AUDIO 权限后开始
  聆听; 实时显示 Partial 结果; 识别完成自动 append 到笔记正文; 错误时
  Toast 提示 (无匹配 / 网络错误 / 权限拒绝等)。
...
### 新增
- **F14 代码块高亮**: 渲染 ```lang ... ``` 围栏代码块, 深色背景 + 语言标签
  + 横向滚动; 自带轻量关键字着色 (Kotlin / Java / Python / JS / Go / Rust / C/C++),
  不引入第三方库; 颜色: 关键字紫 / 字符串橙红 / 数字绿 / 注释灰斜体。
...
### 新增
- **F13 统计仪表盘**: 4 个计数卡 (笔记 / 置顶 / 提醒 / 图片) + 字数卡
  (中文字符 / 英文单词 / 平均每篇) + 分类分布卡 (横向比例条 + 百分比)
  + 月度趋势卡 (最近 6 个月, 竖向柱状图)。数据来自 DB Flow, 实时刷新。
