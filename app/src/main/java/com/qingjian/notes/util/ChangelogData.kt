    ...
    val entries: List<Entry> = listOf(
        Entry(
            ...
            items = listOf(
                ...
                "清理：删除重复资源文件 privacy_policy.md，避免与 privacy_policy.html 产生资源 ID 冲突（Duplicate resources 错误）",
                "",
                "【兼容性优化】",
        ...
        Entry(
            ...
            items = listOf(
                ...
                "新增：笔记归档 — 笔记可标记为「已归档」，列表筛选 Chip 仅显示归档笔记，编辑页「更多」菜单一键切换归档状态",
                "新增：快速分享 — 编辑页「更多」菜单支持调用系统分享，一键分享笔记标题 + 正文纯文本",
                "新增：桌面 Shortcuts — 长按应用图标弹出「新建笔记 / 搜索 / 统计」三个快捷入口",
                "",
                "【数据库 v18 → v19 迁移】",
                "notes 表新增 `is_archived` 列（INTEGER DEFAULT 0），并建立单列索引",
                "",
                "【UI 与交互】",
                ...
                "",
                "升级：版本号 v1.34.00 → v1.35.00 (versionCode 59 → 60)",
            )
        ),
        Entry(
            version = "v1.34.00",
        ...
        Entry(
            ...
            items = listOf(
                ...
                "新增：阅读时间估算 — 笔记编辑页显示阅读时间, 统计页汇总所有笔记的总阅读时间",
                "新增：标签云 / 字数云 — 统计页展示标签与词频分布, 字号根据频率动态映射",
                ...
                "",
                "【数据库 v17 → v18 迁移】",
                "notes 表扩展：新增 is_locked / is_draft / color_tag / read_time_seconds 四个整型列",
                "新增 8 张表：note_backlinks / note_attachments / note_comments / search_history / sync_config / note_change_log / user_note_templates / backlink_scan_state",
        ...
        Entry(
            ...
            items = listOf(
                ...
                "",
                "【数据库与工具】",
                "迁移：v12 → v13 — notes 表新增 template_type 字段 (0=无, 1=日记, 2=会议, 3=读书, 4=周报)",
                "迁移：v13 → v14 — 新增 note_versions 历史版本表 + note_encryption 加密表 (双表外键级联删除)",
        ...
        Entry(
            ...
            date = "2026-06-16",
            items = listOf(
                "优化：vivo 人脸识别适配 — 根据 DC 状态动态选择认证器配置: " +
                    "DC=0 时使用 BIOMETRIC_WEAK | DEVICE_CREDENTIAL (系统显示凭证按钮, 用户可选择人脸); " +
                    "DC≠0 时使用 BIOMETRIC_WEAK | BIOMETRIC_STRONG (显示负向按钮回退到应用内 PIN)",
        ...
        Entry(
            ...
            date = "2026-06-16",
            items = listOf(
                "优化：生物识别认证器配置 — 添加 DEVICE_CREDENTIAL (系统密码/图案/ PIN), " +
                    "使认证器支持范围更完整; 指纹已可用, 人脸识别取决于设备是否通过标准 Biometric API 暴露人脸能力",
        ...
        Entry(
            ...
            date = "2026-06-16",
            items = listOf(
                "修复：MainActivity GlobalScope DelicateCoroutinesApi warning — " +
                    "改用 lifecycleScope.launch(Dispatchers.IO) 启动安全检测, " +
                    "协程绑定到 Activity 生命周期, Activity 销毁时自动取消",
                "修复：AppLockGate 函数缺失导致的编译错误 — 在 MainActivity.kt 末尾补回 AppLockGate Composable, " +
                    "使用 LocalLifecycleOwner + LifecycleEventObserver 监听前后台切换",
                "修复：SecurityChecker 签名校验编译错误 — getSha1Hex() 是 PackageSignatureReader.kt 的顶层函数, " +
                    "直接调用而非 PackageSignatureReader.getSha1Hex()",
        ...
        Entry(
            ...
            date = "2026-06-16",
            items = listOf(
                "安全：NDK Native 层 PIN 哈希 — SHA-256 加盐哈希逻辑从 Kotlin/JVM 移到 C/C++, " +
        ...
        Entry(
            ...
            date = "2026-06-15",
            items = listOf(
                "优化：列表项操作入口 — 移除不可靠的右滑手势 (detectHorizontalDragGestures), " +
                    "改为卡片右上角「⋮」MoreVert 按钮, 点击弹出操作弹层 (置顶/标签/删除/移动/重要度/分享); " +
                    "NoteCard 新增 onMoreClick 可选参数, 列表 items 直接渲染 NoteCard + onMoreClick",
                "修复：NotesListScreen 编译错误 — 清理 SwipeableNoteRow 相关 import 时遗漏 NoteSortOrder, " +
                    "导致 SortOrderDialog 内 9 处引用 Unresolved reference, 补回 import 后恢复正常",
